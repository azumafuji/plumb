        ...
        function parsets(selection) {
            selection.each(function (data, i) {
                ...
                function updateDimensions() {
                    ...
                    tree = buildTree(tree, data, dimensions.map(function (d) {
                        return d.name;
                    }), value_);
                        ...
                        .on("drag", function (d) {
                            ...
                            for (var i = 1; i < dimensions.length; i++) {
                                if (dimensions[i].y < dimensions[i - 1].y) {
                                    ...
                                    dimensionNames = dimensions.map(function (d) {
                                        return d.name;
                                    });
                                    ordinal.domain([]).range(d3.range(dimensions[0].categories.length));
                    ...
                    dimension.select("text").select("tspan.sort.alpha")
                        .on("click", sortBy("alpha", function (a, b) {
                        return a.name < b.name ? 1 : -1;
                    }, dimension));
                    dimension.select("text").select("tspan.sort.size")
                        .on("click", sortBy("size", function (a, b) {
                ...
                function ribbonTweenY(d) {
                    var r = ribbon.filter(function (r) {
